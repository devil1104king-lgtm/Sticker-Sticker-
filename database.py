...
async def init_db():
    ...
    async with await get_db() as db:
        ...
        """)
        # User stats (optional, can be combined with users but separate for clarity)
        await db.execute("""
...
async def register_user(user_id: int, username: str = None, first_name: str = None):
    ...
    async with await get_db() as db:
        ...
        # Also create stats entry if missing
        await db.execute(
...
async def delete_category(name: str) -> bool:
    """Delete category and its stickers (cascade)."""
    async with await get_db() as db:
...
async def add_sticker(category_name: str, file_id: str, file_unique_id: str, added_by: int = 0) -> bool:
    """Add sticker to category. Return True if success."""
    ...
    async with await get_db() as db:
        try:
            ...
            return False  # duplicate file_id
...
async def get_random_sticker(category_name: str) -> Optional[str]:
    """Return file_id of a random sticker from category, or None."""
    category = await get_category_by_name(category_name)
    if not category:
        return None
    async with await get_db() as db:
        # Get all file_ids in category
        rows = await db.execute(
...
async def export_database() -> bytes:
    """Return the entire database as bytes (for backup)."""
    import aiosqlite
    # We'll just read the file
